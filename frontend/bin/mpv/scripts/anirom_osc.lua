...
local user_opts = {
    ...
    seekbarhandlesize = 0.0,    -- size ratio of the diamond and knob handle (0 = thinner, no knob)
    seekrangestyle = "bar",     -- bar, line, slider, inverted or none
    seekrangeseparate = false,  -- whether the seekranges overlay on the bar-style seekbar
    ...
    timecode_color = "#E50914",       -- color of the progress bar and time color
    ...
    held_element_color = "#E50914",   -- color of an element while held down
...
local function bar_layout(direction, slim)
    ...
    geo = { x = osc_geo.x + padX + padwc_l, y = line2, an = 4,
            w = buttonW * 1.5, h = (osc_geo.h - math.abs(line2 - line1)) * 1.5 }
    lo = add_layout("play_pause")
    lo.geometry = geo
    lo.style = osc_styles.smallButtonsBar
    lo.hitbox = { x1 = 0 }

    -- Removed chapter_prev and chapter_next as requested

    -- Left timecode
    geo = { x = geo.x + geo.w + padX + tcW, y = geo.y, an = 6,
            w = tcW, h = (osc_geo.h - math.abs(line2 - line1)) }
    lo = add_layout("tc_left")
    lo.geometry = geo
